async function loadHowtoTranslations(lang) {
  try {
    const response = await fetch(`../js/little_gme_locales/coffee_maker_${lang}.json`);
    if (!response.ok) throw new Error('Not found');
    return await response.json();
  } catch {
    // fallback to English
    const response = await fetch(`../js/little_gme_locales/coffee_maker_en.json`);
    return await response.json();
  }
}
...
async function updateHowtoContent(lang) {
  ...
  const stepsOl = document.getElementById('howto-steps');
  stepsOl.innerHTML = t.steps.map(step => `<li>${step}</li>`).join('');
